...
We need a small, reusable core library that can pass a DOM into accessibility test hooks before we build any host-specific runners, plugins, or UI layers. The first story should prove the contract and a first consumer of `additionalTests` without creating a large, hard-to-review PR.
...
- [ ] The library passes the DOM into caller-provided `additionalTests`.
...
| 03  | `chapter/library-v1/03-dom-hook-orchestration` | Implement readiness gating and make sure `additionalTests` receives the DOM context. | <250 lines |
